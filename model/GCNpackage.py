...
def validate(model, features, adj, labels, idx_val):
    ...
    with torch.no_grad():
        ...
        precision_val, recall_val, f1_val, _ = precision_recall_fscore_support(
            labels[idx_val].cpu(), preds.cpu(), average='weighted'
        )
    return loss_val.item(), accuracy_val, precision_val, recall_val, f1_val
...
def run_gcn(config):
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(np.arange(len(labels)), labels)):
        ...
        for epoch in range(epochs):
            ...
            loss_val, accuracy_val, precision_val, recall_val, f1_val = validate(model, features, adj, labels, idx_val_fold)
            ...
            if (epoch + 1) % 10 == 0:
                print(f"Fold {fold + 1}, Epoch {epoch + 1}: Loss Train {loss_train:.4f}, Loss Val {loss_val:.4f}, Acc Val {accuracy_val:.4f}, "
                      f"pre Val {precision_val:.4f}, recall Val {recall_val:.4f}, F1 Val {f1_val:.4f}")
    ...
    return fold_performance, accuracies, precision,recall,f1
